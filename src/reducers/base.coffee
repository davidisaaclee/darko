...
# progressTimeline = (entityDict, timeline, delta, entities = null) ->
#   if not entities?
#     entities = _.values entityDict

#   newDict = _.assign {}, entityDict,
#     entities
#       .map (e) -> entityDict[e]
#       .map (e) -> _.assign {}, e, data: (Timelines.progress timeline, delta, e.data)
...
reducer = (state = {}, action) ->
  switch action.type
    when k.ProgressTimeline
      ...
      #     dict: _.assign {}, state.entities.dict,
      #       progressTimeline state.entities.dict,
      #         (Scene.getTimeline state, timeline),
      #         delta
    ...
    when k.AttachEntityToTimeline
      ...
      state_ = Scene.mutateEntity state, entity, (e) ->
        Entity.attachTimeline e, timelineObj, progress

      # state_ = mapAssign (_.clone state),
      #   "entities.dict.#{entity}.attachedTimelines",
      #   (oldAttachedTimelines) ->
      #     checkTimeline = (tmln) -> tmln.timeline isnt timeline
      #     isTimelineAlreadyAttached =
      #       not _.all oldAttachedTimelines, checkTimeline
      #     if not isTimelineAlreadyAttached
      #       newAttachedTimeline =
      #         timeline: timeline
      #         progress: progress
